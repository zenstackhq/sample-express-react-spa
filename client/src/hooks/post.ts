...
export function useCheckPost<TError = DefaultError>(args: { operation: PolicyCrudKind; where?: { id?: number; title?: string; content?: string; published?: boolean; authorId?: number }; }, options?: (Omit<UseQueryOptions<boolean, TError, boolean>, 'queryKey'> & ExtraQueryOptions)) {
    const { endpoint, fetch } = getHooksContext();
